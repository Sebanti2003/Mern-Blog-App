  ...
  return (
    <div>
      <Navbar className="border-2 border-slate-300 dark:text-white bg-cyan-200">
        ...
        <form className="flex gap-1">
          <TextInput
            ...
            // rightIcon={IoSearch}
          />
          <Button pill><IoSearch/></Button>
        </form>
        <Button pill className="max-lg:hidden" onClick={() => settheme((theme) => !theme)}>
      ...
      <div className="flex p-2 dark:bg-slate-800 dark:text-white flex-col gap-5 items-center justify-center font-bold">
        <Link className="m-2 w-full h-full text-center" to={`/`}>Home</Link>
        <Link className="m-2 w-full h-full text-center" to={`/about`}>About</Link>
        <Link className="m-2 w-full h-full text-center" to={`/projects`}>Projects</Link>
        <Button outline gradientDuoTone="purpleToBlue" className="mb-2 w-full">Sign In</Button>
        <Button outline gradientDuoTone="purpleToBlue" className="w-full" onClick={() => settheme((theme) => !theme)}>
          {theme ? <IoMoon /> : <IoSunny />}
        </Button>
